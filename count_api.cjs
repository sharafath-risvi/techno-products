...
async function fetchAll() {
  ...
  for (const product of allProducts) {
    if (product.id === 10390) continue; // Drop the extra Danfoss drive with no category
    ...
    } else if (/\bMOTOR(S|L)?\b/i.test(combinedText)) { // Fixed to catch Motorl
  ...
  console.log("Total:", allProducts.length - 1);
  console.log(counts);
  console.log("Switchgears:", switchgears);
}
