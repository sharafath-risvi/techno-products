...
async function fetchAll() {
  ...
  for (const product of allProducts) {
    ...
    if (/\bMOTOR(S)?\b/i.test(combinedText)) { 
      if (combinedText.includes("INVERTER") || combinedText.includes("CONVERTER") || combinedText.includes("SINAMICS")) {
          console.log(`FOUND DRIVE IN MOTORS: [${product.id}] ${product.name}`);
      }
    }
  }
}
